...
任何内容在网络中的发布与传播，至少存在 3 个核心角色：

- `Owner / Author`：内容的创造者或权利主体。
- `Curator / 收录者`：决定把内容纳入某个目录、集合、榜单或上下文的人。
- `Sharer / 传播者`：决定把内容沿某条社交链、消息链或引用链继续扩散的人。

`Content 的传播证明` 的真正目标，就是把这 3 类角色留下的高价值行为都表达成可验证对象。  
