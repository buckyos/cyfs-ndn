...
- Zone 内一个进程怎么访问 Named File Manager
- 当数据已经确定落到某个 store bucket 后，怎么执行最薄的一层对象/Chunk 读写
...
> 设备上的一个进程，应该用什么协议访问 Zone 内的 Named File Manager

典型调用链是：

`Device -> ZoneGateway -> NamedFileMgr`
...
`readme` 里的 `NamedFileMgr` 更适合被理解为“Zone 内 Named Data 能力”的概念性表述。
...
在 Zone 内，gateway 负责把设备侧请求映射到 `NamedFileMgr` 的上传面和控制面。  
...
后者虽然语义上属于 CYFS 层，但在当前代码实现里，`NdnDirServer` 也是直接持有 `NamedStoreMgr`，没有先经过 `NamedFileMgr` 再下沉到 `store_mgr`。
...
flowchart LR
    ...
    ZoneGw --> Mgr["NamedStoreMgr\nsrc/named_store/src/store_mgr.rs"]
    Dir --> Mgr
...
2. gateway 调用 NamedFileMgr，根据 layout 选择目标 bucket
