use crate::{NdnError, NdnResult, ObjId, PathObject};
...
pub fn cyfs_get_obj_id_from_url(cyfs_url: &str) -> NdnResult<(ObjId, Option<String>)> {
    let url = Url::parse(cyfs_url)
        .map_err(|e| NdnError::InvalidId(format!("parse cyfs url failed:{}", e.to_string())))?;
    let host = url.host_str();
    if host.is_none() {
        return Err(NdnError::InvalidId(format!(
            "cyfs url host not found:{}",
            cyfs_url
        )));
    }
    ...
    if chunk_size.is_some() {
        ...
        let chunk_size = chunk_size.parse::<u64>().map_err(|e| {
            NdnError::DecodeError(format!(
                "get chunk size from headers failed:{}",
                e.to_string()
            ))
        })?;
